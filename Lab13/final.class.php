 ...
 */
...

	{
		try {
			...
			$retData["status"]=0;
			$retData["message"]="insert of '$MapJson' for location: '$Location' and WeatherJson '$WeatherJson' accepted";
		}
		catch  (Exception $e) {
			$retData["status"]=1;
			$retData["message"]=$e->getMessage();
...

	{
		try {
			...
			$retData["status"]=0;
			$retData["message"]="select of all weather columns matching date  accepted";
		}
		catch  (Exception $e) {
			$retData["status"]=1;
			$retData["message"]=$e->getMessage();
